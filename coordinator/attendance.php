    ...
    <body>
        <div class="p-3">
            <div class="p-3 border border-2 rounded-3">
                <header>
                    <h3 class="h3 fw-bold"><?php echo htmlentities($subName) ?> Attendance</h3>
                    <hr class="hr">
                </header>
        ...
        <script>
            let openClass = ()=>{
                window.open(`attendance.php?subjectId=<?php echo htmlentities($subId)?>&subjectName=<?php echo htmlentities($subName)?>&class=${document.getElementById('cls').value}`,"_self");
            }
        </script>
    </body>
            ...
            <div class="p-3 border border-2 rounded-3">
                ...
                <table id="Table" class="table table-hover caption-top">
                    ...
                    <tbody class="table-group-divider">
                        <?php
                        $students = mysqli_fetch_all(mysqli_query($con, "SELECT `$studId`.`id` ,`$studId`.`student id` ,`$studId`.`attendance`,`student`.`student name`,`student`.`class` FROM `$studId` INNER JOIN `student` ON `$studId`.`student id`=`student`.`student id` WHERE `student`.`class`='$clas'"), MYSQLI_ASSOC);
